...
class VCOCOeval(object):
  ...
  def _do_eval(self, dets, ovr_thresh=0.5, model_name='test'):

    # self._do_agent_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh)
    self._do_role_eval(self.vcocodb, dets, ovr_thresh=ovr_thresh, eval_type='scenario_1', model_name=model_name)
    # self._do_role_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh, eval_type='scenario_2')


  def _do_role_eval(self, vcocodb, dets, ovr_thresh=0.5, eval_type='scenario_1', model_name='test'):
    ...
    print('---------------------------------------------')
    LOCAL_DATA = '/public/data0/users/houzhi28/data0/'
    iter_str = model_name.split('_')[0]
    iter_str = iter_str.replace('/', '')
    model_name = model_name[len(iter_str)+1:]
    model_name = model_name.replace('.pkl', '')
    f = open(LOCAL_DATA +  '/coco_csv/{}_{}.csv'.format(model_name, eval_type), 'a')
    print(LOCAL_DATA +  '/coco_csv/{}_{}.csv'.format(model_name, eval_type), 'a')
    f.write('%.2f %.2f\n'%(
