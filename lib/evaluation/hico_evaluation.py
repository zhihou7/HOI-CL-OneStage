...
class HICOEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            if "hoi_instances" in output:
                ...
                prediction["hoi_instances"] = instances_to_hico_matlab(self._metadata, instances, self.is_hoi_prediction)
    ...
    def _evaluate_hico_on_matlab(self, dets_file, anno_file, bbox_file):
        ...
        if self._matlab == 'octave':
            cmd += '{:s} --eval '.format(self._matlab)
            cmd += '"dbstop if error; '
        else:
...
def instances_to_hico_matlab(metadata, instances, is_hoi_prediction=False):
    """
